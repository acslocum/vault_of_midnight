...
The application pairs a "trigger" object with a "player" object to provide a highly configurable way to play Fortress Party off from some sort of action taken by a guest. The **config.ini** file is the key to setting everything up, so we'll start there.

## config.ini
The config file is parsed by Python's configparser module. It's a standard .ini file structure - sections can be created via placing the section name in square brackets like so:  
```
[general]
```  
